...
// Mock firebaseAdmin.js for testing Firebase DB layer (Firebase, 2019d)
jest.unstable_mockModule("../db/firebaseAdmin.js", () => ({
  default: {
    database: () => ({
      ref: () => ({
        get: jest.fn(async () => ({ exists: () => true })), // Simulates a Firestore/Realtime DB snapshot (Firebase, 2019a; 2019d)
      }),
    }),
  },
}));

// Import after applying module mock (Nakazawa Tech, 2018)
const admin = (await import("../db/firebaseAdmin.js")).default;

// Jest test for verifying Firebase database connection mock (Firebase, 2019d)
test("connects to firebase db (mock ok)", async () => {
  const db = admin.database(); // Mocked Realtime DB instance (Firebase, 2019d)
  const snap = await db.ref("test/ping").get(); // Simulated get() request to path
  expect(snap.exists()).toBe(true); // (Firebase, 2019d)
});
